<template>
  <ion-footer>
    <ion-toolbar class="ion-padding-vertical">
      ...
      <ion-buttons v-if="!isUpdate" slot="end">
        <ion-button v-if="resetPass" color="primary" @click="$emit('reset')">
          <ion-icon :icon="icons.refresh" />
          <ion-label>Reset Sandi</ion-label>
        </ion-button>
        <ion-button v-if="setAdmin" :color="adminColor" @click="$emit('admin')">
          <ion-icon :icon="adminIcon" />
          <ion-label>{{ adminLabel }}</ion-label>
        </ion-button>
      </ion-buttons>
...
export default defineComponent({
  ...
  emits: ["update", "trash", "cancel", "send", "reset", "admin"],
  props: {
  ...
  setup(props) {
    ...
    const adminIcon = computed(() => {
      return props.isAdmin == 1 ? personRemove : personAdd;
    });
    const adminLabel = computed(() => {
      return props.isAdmin == 1 ? "Hapus Admin" : "Tambah Admin";
    });
    const adminColor = computed(() => {
      return props.isAdmin == 1 ? "danger" : "primary";
    });
    const icons = ref({
      send,
      pencil,
    ...
    return { icons, isUpdate, adminIcon, adminLabel, adminColor };
  },
});
</script>
