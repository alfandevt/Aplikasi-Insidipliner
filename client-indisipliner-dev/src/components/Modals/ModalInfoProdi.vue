<template>
  ...
  <ion-content :fullscreen="true">
    <form-prodi :prodi-label="itemObj.prodi" :form-mode="formMode" />
  </ion-content>
...
import FormProdi, { ProdiFormComponent } from "../Forms/FormProdi.vue";
...
export default defineComponent({
  components: { IonContent, ModalHeader, FormProdi, ModalItemCrudFooter },
  props: {
    title: { type: String, default: "Data Prodi" },
  ...
  setup(props) {
    ...
    const currentForm = useForm({ validationSchema: ProdiFormComponent });
    ...
    function trashData(value) {
      ...
      http
        .delete("/api/akademik/prodi/delete", body, config)
        .then((response) => {
    ...
    function submitData(body) {
      const config = appJsonToken();
      http
        .put("/api/akademik/prodi/update", body, config)
        .then((response) => {
          const data = { ...response.data };
          baseToast(data.message, "success");
          props.submitValue();
