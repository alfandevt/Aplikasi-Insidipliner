<template>
  ...
  <ion-content :fullscreen="true">
    <form-kelas
      :kelas-label="itemObj.kelas"
      :wali-kelas-id="itemObj.wali_kelas_id"
      :wali-kelas-label="itemObj.wali_kelas"
      :jurusan-id="itemObj.jurusan_id"
      :jurusan-label="itemObj.jurusan"
      :periode-id="itemObj.periode_id"
      :periode-label="itemObj.periode"
      :form-mode="formMode"
    />
  </ion-content>
...
import FormKelas, { KelasFormComponent } from "../Forms/FormKelas.vue";
...
export default defineComponent({
  components: { IonContent, ModalHeader, FormKelas, ModalItemCrudFooter },
  props: {
  ...
  setup(props) {
    ...
    const currentForm = useForm({ validationSchema: KelasFormComponent });
    ...
    function trashData(value) {
      ...
      http
        .delete("/api/akademik/kelas/delete", body, config)
        .then((response) => {
    ...
    function submitData(body) {
      const config = appJsonToken();
      http
        .put("/api/akademik/kelas/update", body, config)
        .then((response) => {
          const data = { ...response.data };
          baseToast(data.message, "success");
          props.submitValue();
