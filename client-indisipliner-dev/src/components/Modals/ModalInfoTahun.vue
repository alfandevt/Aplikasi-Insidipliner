<template>
  ...
  <ion-content :fullscreen="true">
    <form-tahun-ajaran
      :tahun-awal="itemObj.tahun_mulai"
      :tahun-akhir="itemObj.tahun_selesai"
      :form-mode="formMode"
    />
  </ion-content>
...
import FormTahunAjaran, { TahunFormComponent } from "../Forms/FormTahunAjaran.vue";
...
export default defineComponent({
  components: { IonContent, ModalHeader, FormTahunAjaran, ModalItemCrudFooter },
  props: {
    title: { type: String, default: "Data Tahun Akademik" },
  ...
  setup(props) {
    ...
    const currentForm = useForm({ validationSchema: TahunFormComponent });
    ...
    function trashData(value) {
      ...
      http
        .delete("/api/akademik/tahun/delete", body, config)
        .then((response) => {
    ...
    function submitData(body) {
      const config = appJsonToken();
      http
        .put("/api/akademik/tahun/update", body, config)
        .then((response) => {
          const data = { ...response.data };
          baseToast(data.message, "success");
          props.submitValue();
