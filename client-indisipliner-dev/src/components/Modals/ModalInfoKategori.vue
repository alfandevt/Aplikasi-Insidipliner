<template>
  ...
  <ion-content :fullscreen="true">
    <form-kategori
      :kategori-label="itemObj.kategori"
      :poin-number="itemObj.poin"
      :form-mode="formMode"
    />
  </ion-content>
...
import FormKategori, { KategoriFormComponent } from "../Forms/FormKategori.vue";
...
export default defineComponent({
  components: { IonContent, ModalHeader, FormKategori, ModalItemCrudFooter },
  props: {
    title: { type: String, default: "Data Kategori" },
  ...
  setup(props) {
    ...
    const currentForm = useForm({ validationSchema: KategoriFormComponent });
    ...
    function trashData(value) {
      ...
      http
        .delete("/api/pelanggaran/kategori/delete", body, config)
        .then((response) => {
    ...
    function submitData(body) {
      const config = appJsonToken();
      http
        .put("/api/pelanggaran/kategori/update", body, config)
        .then((response) => {
          const data = { ...response.data };
          baseToast(data.message, "success");
          props.submitValue();
