<template>
  ...
  <ion-content :fullscreen="true">
    <form-bidang :bidang-label="itemObj.bidang" :form-mode="formMode" />
  </ion-content>
...
import FormBidang, { BidangFormComponent } from "../Forms/FormBidang.vue";
...
export default defineComponent({
  components: { IonContent, ModalHeader, FormBidang, ModalItemCrudFooter },
  props: {
    title: { type: String, default: "Data Bidang" },
  ...
  setup(props) {
    ...
    const currentForm = useForm({ validationSchema: BidangFormComponent });
    ...
    function trashData(value) {
      ...
      http
        .delete("/api/petugas/bidang/delete", body, config)
        .then((response) => {
    ...
    function submitData(body) {
      const config = appJsonToken();
      http
        .put("/api/petugas/bidang/update", body, config)
        .then((response) => {
          const data = { ...response.data };
          baseToast(data.message, "success");
          props.submitValue();
