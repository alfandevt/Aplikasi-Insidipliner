  ...
  <ion-page>
    ...
    <ion-content :fullscreen="true">
      ...
      <base-list v-if="!listDataNull" keyName="row_id" v-slot="{ item }">
        <base-item :title="item['bidang']" @btnClick="openInfoModal(item)" />
      </base-list>
...
import ModalCreateBidang from "../../../components/Modals/ModalCreateBidang";
import ModalInfoBidang from "../../../components/Modals/ModalInfoBidang";
...
export default defineComponent({
  ...
  setup() {
    ...
    async function openCreateModal() {
      await openModal(ModalCreateBidang, {
        title: "Form Bidang",
        dismiss: closeModal,
    ...
    async function openInfoModal(obj) {
      await openModal(ModalInfoBidang, {
        title: "Data Bidang",
        dismiss: closeModal,
        /* user props */
        itemObj: obj,
    ...
    function loadFeed(keyword) {
      ...
      http
        .get("/api/petugas/bidang/list", params)
        .then((response) => {
